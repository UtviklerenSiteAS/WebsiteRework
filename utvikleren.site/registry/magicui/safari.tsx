...
function IframePreview({ src, baseWidth }: { src: string; baseWidth: number }) {
    ...
    <div ref={ref} className="w-full h-full overflow-hidden">
      {scale > 0 && (
        <iframe
          ...
          style={{
            width: `${baseWidth}px`,
            height: containerH > 0 ? `${Math.ceil(containerH / scale)}px` : "100%",
...
export function Safari({
  ...
  const hasMedia = hasVideo || !!imageSrc || !!iframeSrc;
    ...
    >
      ...
      {!hasVideo && !imageSrc && iframeSrc && (
        <div
          ...
          style={{ left: `${LEFT_PCT}%`, top: `${TOP_PCT}%`, width: `${WIDTH_PCT}%`, height: `${HEIGHT_PCT}%`, borderRadius: "0 0 11px 11px" }}
        >
          <IframePreview src={iframeSrc} baseWidth={1200} />
        </div>
      )}
      ...
      >
        ...
        <g clipPath="url(#path0)" mask={hasMedia ? "url(#safariPunch)" : undefined}>
          ...
          {url && (
            <text x="585" y="30" fill="#A3A3A3" fontSize="12" fontFamily="ui-sans-serif, system-ui, sans-serif">{url}</text>
          )}
          {mode === "default" && (
            <>
              <g className="mix-blend-luminosity">
                <path d="M143.914 32.5938C144.094 32.7656 144.312 32.8594 144.562 32.8594C145.086 32.8594 145.492 32.4531 145.492 31.9375C145.492 31.6797 145.391 31.4453 145.211 31.2656L139.742 25.9219L145.211 20.5938C145.391 20.4141 145.492 20.1797 145.492 19.9219C145.492 19.3984 145.086 19 144.562 19C144.312 19 144.094 19.0938 143.914 19.2734L137.633 25.3359C137.414 25.5469 137.305 25.7266 137.305 25.9219C137.305 26.1172 137.414 26.3281 137.633 26.5391L143.914 32.5938Z" fill="#A3A3A3" />
              </g>
              <g className="mix-blend-luminosity">
                <path d="M168.422 32.8594C168.68 32.8594 168.891 32.7656 169.07 32.5938L175.148 26.5391C175.359 26.3281 175.469 26.1172 175.469 25.9219C175.469 25.7266 175.359 25.5469 175.148 25.3359L169.07 19.2734C168.891 19.0938 168.68 19 168.422 19C167.906 19 167.492 19.3984 167.492 19.9219C167.492 20.1797 167.602 20.4141 167.773 20.5938L173.242 25.9219L167.773 31.2656C167.602 31.4453 167.492 31.6797 167.492 31.9375C167.492 32.4531 167.906 32.8594 168.422 32.8594Z" fill="#A3A3A3" />
              </g>
              <g className="mix-blend-luminosity">
